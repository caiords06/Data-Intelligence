"""Compatibilidade do correio V9 legado.

A implementação canônica é :mod:`enterprise.correio`, usada pelo servidor
corporativo atual. Este módulo permanece somente para migração/compatibilidade
com instalações V9 e não deve receber novos recursos de produto.
"""
...
def enviar_mensagem(dados: dict, ator: dict, *, rascunho: bool = False) -> int:
            ...
            for origem in anexos:
                ...
                conexao.execute(
                    ...
                    (
                        ...
                        _hash(destino), mimetypes.guess_type(origem.name)[0],
                    ),
                )
            if status == "Enviada":
    ...
    registrar_auditoria(
        ...
        modulo="comunicacao", entidade="mensagens", entidade_id=int(mensagem_id),
    )
