...
def _upload_streaming(p: Path, endpoint: str, *, headers: dict, timeout: int) -> dict:
    ...
    try:
        ...
        if resposta.status >= 400:
            try: msg=json.loads(raw.decode("utf-8")).get("erro")
        ...
        return dict(json.loads(raw.decode("utf-8"))) if "application/json" in ctype else {"conteudo":raw}
    except OSError as erro:
        raise ConnectionError(f"Não foi possível enviar o arquivo ao servidor corporativo: {erro}") from None
    finally:
        conexao.close()
