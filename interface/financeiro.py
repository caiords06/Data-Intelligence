...
class TelaFinanceiro:
    ...
    def _criar_interface(self):
        ...
        grupos.append(("COLABORAÇÃO", (("correio", "✉", "Correio interno", lambda: self.navegacao["correio"]("financeiro")),)))
        criar_sidebar(
            self.container,
            self.navegacao,
            ativo=self.secao,
            grupos_customizados=tuple(grupos),
            titulo_customizado="FINANCEIRO",
    ...
    def _visao_geral(self):
        ...
        tk.Label(linha, text="PERÍODO", font=("Segoe UI", 8, "bold"), fg=CORES["text_sec"], bg=CORES["card"]).pack(side="left")
    ...
    def _atalhos(self):
        ...
        for icone, titulo, descricao, comando in acoes:
            ...
            tk.Label(bloco, text=titulo, font=("Segoe UI", 9, "bold"), fg=CORES["text"], bg=CORES["card_secundario"]).pack(anchor="w", padx=13)
    ...
    def _ciclo_financeiro(self):
        ...
        for etapa, destino in etapas:
            botao = tk.Button(
                ...
                font=("Segoe UI", 8, "bold"), fg=CORES["text_sec"], bg=CORES["input"],
    ...
    def _livro(self):
        ...
        def campo(rotulo, variavel, *, valores=None, pesquisa=False):
            ...
            tk.Label(
                grupo, text=rotulo.upper(), font=("Segoe UI", 7, "bold"),
    ...
    def _janela_detalhes(self, identificador):
        ...
        for indice, (rotulo, valor) in enumerate(dados):
            ...
            tk.Label(bloco, text=rotulo.upper(), font=("Segoe UI", 8, "bold"), fg=CORES["text_muted"], bg=CORES["card_secundario"]).pack(anchor="w", padx=12, pady=(8, 2))
    ...
    def _modal_formulario(self, titulo, campos, padroes, mapas, salvar):
        ...
        for indice, definicao in enumerate(campos):
            ...
            tk.Label(grupo, text=rotulo.upper(), font=("Segoe UI", 8, "bold"), fg=CORES["text_sec"], bg=CORES["card"]).pack(anchor="w", pady=(0, 5))
            if tipo == "bool":
    ...
    def _gerar_relatorio(self, tipo, formato):
        try:
            caminho = gerar_relatorio_financeiro(tipo, formato, SESSAO.usuario)
        except (ValueError, PermissionError, RuntimeError) as erro:
            messagebox.showerror("Relatório", str(erro), parent=self.root)
            return
        messagebox.showinfo("Relatório concluído", f"Arquivo criado em:\n{caminho}", parent=self.root)
    ...
    def _mostrar_analise(self):
        ...
        texto.tag_configure("titulo", foreground=COR_FINANCEIRO, font=("Segoe UI", 10, "bold"), spacing1=6, spacing3=5)
        texto.configure(state="disabled")
        rodape = tk.Frame(janela, bg=CORES["bg"])
        rodape.pack(fill="x", padx=26, pady=18)
    ...
    def _form_simples_valor(self, titulo, rotulo):
        ...
        tk.Label(janela, text=rotulo.upper(), font=("Segoe UI", 8, "bold"), fg=CORES["text_sec"], bg=CORES["bg"]).pack(anchor="w", padx=24)
        var = tk.StringVar()
